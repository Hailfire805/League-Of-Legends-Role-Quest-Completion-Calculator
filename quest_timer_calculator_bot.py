...
def generate_accumulation_curve(cs_per_min_bot, cs_per_min_other, plates_bot, 
    ...
    for i, t in enumerate(time_points):
        ...
        if completion_time > 0:
            ...
            points += calculate_points_from_cs(cs_bot_at_time, cs_other_at_time)
...
class BotLaneQuestCalculatorGUI:
    ...
    def setup_ui(self):
        ...
        input_frame = ttk.LabelFrame(main_frame, text="BOT LANE Performance Metrics", padding="10")
        ...
        ttk.Label(input_frame, text="CS/min in Bot Lane:").grid(row=row, column=0, sticky=tk.W, pady=5)
        self.cs_per_min_bot = tk.StringVar(value="7.0")
        ttk.Entry(input_frame, textvariable=self.cs_per_min_bot, width=15).grid(row=row, column=1, pady=5)
        row += 1
        ...
        ttk.Label(input_frame, text="Plates in Bot Lane (0-5):").grid(row=row, column=0, sticky=tk.W, pady=5)
        self.plates_bot = tk.StringVar(value="0")
        ttk.Spinbox(input_frame, from_=0, to=5, textvariable=self.plates_bot, width=13).grid(row=row, column=1, pady=5)
        row += 1
        ...
        ttk.Label(input_frame, text="Turrets in Bot Lane:").grid(row=row, column=0, sticky=tk.W, pady=5)
        self.turrets_bot = tk.StringVar(value="0")
        ttk.Entry(input_frame, textvariable=self.turrets_bot, width=15).grid(row=row, column=1, pady=5)
        row += 1
    ...
    def plot_baseline(self):
        ...
        self.ax.plot(time_points, percentage_complete, '--', color='gray', 
                    label='Passive Only (Bot Lane)', linewidth=2)
        ...
        self.ax.set_title('BOT LANE Quest - Completion Progress', fontsize=14, fontweight='bold')
        self.ax.legend()
